...
import collect_v5_3 as v5_3

KST = base.KST
ROOT = Path(__file__).resolve().parent
STATE_DIR = ROOT / 'state'
REPORT_DIR = ROOT / 'reports_v5_3_1'
...
def stronger_content_signature(item: dict, meta: dict) -> str:
    filenames = sorted({p.lower() for p in (item.get('file_tree', []) or [])})[:120]
    readme = (item.get('readme', '') or '')
    readme_head = readme[:2000]
    readme_tail = readme[-800:] if len(readme) > 800 else readme
    artifact_summaries = []
    for art in (item.get('artifacts', []) or [])[:5]:
        artifact_summaries.append(f"{art.get('type','')}::{art.get('path','')}::{(art.get('content','') or '')[:300]}")
    parts = [
        str(item.get('week', '')),
        item.get('repo_name', ''),
...
def update_observations(classified: list[dict]) -> tuple[dict, list[dict]]:
    ...
    for item in classified:
        ...
        course = item['course']
        sig = stronger_content_signature(item, course)
...
def build_report(all_items: list[dict], classified: list[dict], new_repos: list[dict], hygiene: dict, overwritten_flags: list[dict], candidate_pool_size: int) -> tuple[str, dict]:
    ...
    lines = []
    lines.append(f'# Codyssey registry-backed collection report v5.3.1 ({date_str})')
    lines.append('')
    ...
    lines.append(f'- signature version: v5.3.1')
...
def main() -> None:
    print(f"=== Codyssey v5.3.1 수집 시작 ({datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')}) ===")
