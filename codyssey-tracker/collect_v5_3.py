...
def content_signature(item: dict, meta: dict) -> str:
    filenames = sorted({p.split('/')[-1].lower() for p in (item.get('file_tree', []) or [])})[:80]
    parts = [
        str(item.get('week', '')),
        item.get('repo_name', ''),
        str(item.get('readme_length', 0)),
        '|'.join(filenames),
        '|'.join(meta.get('evidence', [])[:5]),
    ]
...
def update_observations(classified: list[dict]) -> tuple[dict, list[dict]]:
    ...
    for item in classified:
        ...
        course = item['course']
        sig = content_signature(item, course)
...
def build_report(all_items: list[dict], classified: list[dict], new_repos: list[dict], hygiene: dict, overwritten_flags: list[dict], candidate_pool_size: int) -> tuple[str, dict]:
    ...
    lines = []
    lines.append(f'# Codyssey registry-backed collection report v5.3 ({date_str})')
    lines.append('')
    ...
    if overwritten_flags:
        lines.append('| 레포 키 | 이전 주차 | 현재 주차 | 이전 상태 | 현재 상태 | 서명 변경 |')
        lines.append('| --- | --- | --- | --- | --- | --- |')
        for flag in overwritten_flags:
            lines.append(f"| {flag['repo_key']} | {flag.get('previous_week','-')} | {flag.get('current_week','-')} | {flag.get('previous_status','-')} | {flag.get('current_status','-')} | {'Y' if flag.get('signature_changed') else 'N'} |")
        lines.append('')
        lines.append('같은 레포에 주차별 내용을 덮어쓰는 경우를 대비해 `state/repo_observations.json`에 관측 이력을 누적합니다.')
        lines.append('')
    else:
        lines.append('_이번 실행에서는 뚜렷한 덮어쓰기/변조 의심 레포가 없었습니다._')
        lines.append('')

    lines.append('## 3. 후보/검토 레포 (active watchlist)')
    lines.append('')
    if by_bucket:
...
def main() -> None:
    print(f"=== Codyssey v5.3 수집 시작 ({datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')}) ===")
    ...
    report_text, payload = build_report(all_items, classified, new_repos, hygiene, overwritten_flags, len(candidates))
    date_str = datetime.now(KST).strftime('%Y-%m-%d')
